// :sentinel-audio — capability module. Compiled into the consumer build
// ONLY when the `audio` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/audio for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/audio")
    else file("$projectDir/../../builds/audio")
)

android {
    namespace = "com.mobilesentinel.audio"
    compileSdk = 34
