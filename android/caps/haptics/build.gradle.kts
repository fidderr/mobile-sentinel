// :sentinel-haptics — capability module. Compiled into the consumer build
// ONLY when the `haptics` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/haptics for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/haptics")
    else file("$projectDir/../../builds/haptics")
)

android {
    namespace = "com.mobilesentinel.haptics"
    compileSdk = 34
