// :sentinel-scanner — capability module. Compiled into the consumer build
// ONLY when the `scanner` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/scanner for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/scanner")
    else file("$projectDir/../../builds/scanner")
)

android {
    namespace = "com.mobilesentinel.scanner"
    compileSdk = 34
