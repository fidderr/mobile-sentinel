// :sentinel-overlay — capability module. Compiled into the consumer build
// ONLY when the `overlay` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/overlay for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/overlay")
    else file("$projectDir/../../builds/overlay")
)

android {
    namespace = "com.mobilesentinel.overlay"
    compileSdk = 34
