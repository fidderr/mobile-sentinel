// :sentinel-network — capability module. Compiled into the consumer build
// ONLY when the `network` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/network for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/network")
    else file("$projectDir/../../builds/network")
)

android {
    namespace = "com.mobilesentinel.network"
    compileSdk = 34
