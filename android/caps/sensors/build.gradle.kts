// :sentinel-sensors — capability module. Compiled into the consumer build
// ONLY when the `sensors` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/sensors for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/sensors")
    else file("$projectDir/../../builds/sensors")
)

android {
    namespace = "com.mobilesentinel.sensors"
    compileSdk = 34
