// :sentinel-display — capability module. Compiled into the consumer build
// ONLY when the `display` capability (Cargo feature) is enabled.
plugins {
...
// back to the in-tree android/builds/display for workspace/path-dependency dev.
val sentinelBuildRoot = project.findProperty("sentinelBuildRoot") as String?
layout.buildDirectory.set(
    if (sentinelBuildRoot != null) file("$sentinelBuildRoot/display")
    else file("$projectDir/../../builds/display")
)

android {
    namespace = "com.mobilesentinel.display"
    compileSdk = 34
